...
var DemoList = class extends import_coc.BasicList {
  constructor(nvim) {
    super(nvim);
    this.name = "demo_list";
    this.description = "CocList for coc-deepl";
  ...
  async loadItems(context) {
    return [
      {
        label: "coc-deepl list item 1",
        data: {name: "list item 1"}
      },
      {
        label: "coc-deepl list item 2",
...
async function activate(context) {
  import_coc2.window.showMessage(`coc-deepl works!`);
  context.subscriptions.push(import_coc2.commands.registerCommand("coc-deepl.Command", async () => {
    import_coc2.window.showMessage(`coc-deepl Commands works!`);
  }), import_coc2.listManager.registerList(new lists_default(import_coc2.workspace.nvim)), import_coc2.sources.createSource({
    name: "coc-deepl completion source",
    doComplete: async () => {
      const items = await getCompletionItems();
      return items;
    }
  }), import_coc2.workspace.registerKeymap(["n"], "deepl-keymap", async () => {
